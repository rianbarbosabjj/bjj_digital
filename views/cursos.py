...
Integração com utils.py, Uploads, Editores (Busca) e Design Rico.
"""
...
def _professor_listar_cursos(usuario):
    ...
    if not cursos:
        ...
        <p style="opacity: 0.7;">Você ainda não criou nenhum curso.</p>
        </div>
        """, unsafe_allow_html=True)
        return
...
def _exibir_detalhes_curso(curso, usuario):
    ...
    with c_info:
        ...
        if modulos:
            for m in modulos:
                with st.expander(f"{m['titulo']} ({len(m['aulas'])})"):
                    for a in m['aulas']: st.caption(f"- {a['titulo']}")
        else: st.info("Em breve.")
    ...
    with c_acao:
        ...
        if can_edit:
            if st.button("✏️ Editar", use_container_width=True): navegar_para('edicao', curso)
            if st.button("➕ Gerenciar", type="primary", use_container_width=True): navegar_para('gerenciar_conteudo', curso)
        ...
        for m in modulos:
            with st.expander(m['titulo'], expanded=True):
                for a in m['aulas']:
                    ...
                    # Usando markdown para destacar aula atual
                    label = a['titulo']
...
def _aluno_cursos_disponiveis(usuario):
    ...
    for i, c in enumerate(cursos):
        with cols[i%3]:
            # Reutiliza o card visual (sem os botões de edição)
            pago = c.get('pago', False)
            icon = "💎" if pago else "🎓"
