...
def carregar_exame_especifico(faixa_alvo):
    ...
    # 2. Se não achou questões, busca no banco
...
def exame_de_faixa(usuario):
    ...
    # 2. Datas (Com Fuso Horário UTC-3)
    try:
        data_inicio = dados.get('exame_inicio')
        data_fim = dados.get('exame_fim')
        
        # Usa UTC puro e subtrai 3h para garantir horário de Brasília fixo
        ...
        if data_fim and agora > data_fim:
            ...
            # Debug para o aluno entender
            st.caption(f"Horário atual do sistema (Brasília): {agora.strftime('%d/%m/%Y %H:%M')}")
            return
    except: pass
    ...
    if not st.session_state.exame_iniciado:
        ...
        with st.container(border=True):
            ...
            st.markdown("---")
            st.markdown("**ATENÇÃO:** Não saia da tela.")
