...
def gestao_exame_de_faixa():
    ...
    # ---------------------------------------------------------
    # ABA 1: MONTAR PROVA (Com Visualização Melhorada)
    # ---------------------------------------------------------
    with tab_prova:
                    ...
                    if q['pergunta'] not in perguntas_no_exame:
                        ...
                        with c_det:
                            ...
                            if opcoes:
                                ...
                                st.caption("⚠️ (Sem alternativas cadastradas)")
            ...
            if 'questoes' in exame and exame['questoes']:
                for i, q in enumerate(exame['questoes']):
                    with st.expander(f"{i+1}. {q['pergunta']} ({q.get('tema','?')})"):
                        # Detalhes dentro do expander para economizar espaço na lista final
                        if 'opcoes' in q:
                            for op in q['opcoes']:
                                st.write(f"• {op}")
                ...
                st.info("Nenhuma questão adicionada a este exame ainda.")
    ...
    with tab_alunos:
        ...
        if user_logado['tipo'] == 'admin':
            ...
            if not equipes_permitidas:
                st.warning("Você não possui equipes vinculadas.")
                return
