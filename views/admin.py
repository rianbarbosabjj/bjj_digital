...
# =========================================
# GESTÃO DE EXAME DE FAIXA (SEPARADO E ORGANIZADO)
# =========================================
def gestao_exame_de_faixa():
    ...
    # AGORA SÃO 3 ABAS PARA ORGANIZAR MELHOR
    ...
    # ---------------------------------------------------------
    # ABA 2: VISUALIZAR (Separado por Abas de Cor)
    # ---------------------------------------------------------
    with tab_visualizar:
        st.subheader("Visualizar Provas Cadastradas")
        st.caption("Consulte aqui o conteúdo das provas já criadas.")
        
        abas_cores = st.tabs(faixas)
        
        for aba, f_nome in zip(abas_cores, faixas):
            with aba:
        ...
        if not docs_alunos:
            ...
            for doc in docs_alunos:
                ...
                if equipes_permitidas is None or eid in equipes_permitidas:
                    ...
                    try: idx_f = faixas.index(lib)
                    except: idx_f = 0
                    fx_sel = c[2].selectbox("Faixa", faixas, index=idx_f, key=f"s_{doc.id}", label_visibility="collapsed")
                    c[3].caption(status_txt)
                    
                    if hab:
                        if c[4].button("⛔", key=f"b_{doc.id}"):
                            db.collection('alunos').document(doc.id).update({"exame_habilitado": False})
                            st.rerun()
                    else:
                        if c[4].button("✅", key=f"l_{doc.id}"):
